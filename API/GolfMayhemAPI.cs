...
namespace GolfMayhem.API
{
    ...
    public static class GolfMayhemAPI
    {
        ...
        // ── Manual Triggers (useful for testing or custom triggers) ──

        /// <summary>
        /// Show a custom announcement on the AnnouncerHUD.
        /// Safe to call even if AnnouncerHUD is disabled — it silently ignores.
        /// </summary>
        public static void ShowAnnouncement(string message, Color color)
        {
            AnnouncerHUD.Instance?.ShowAnnouncement(message, color);
        }

        public static void ShowAnnouncement(string message)
        {
            ShowAnnouncement(message, Color.white);
        }
    }
}
